...
public class TFImageUtils {
    ...
    private static final String TAG = "TFImageUtils";
    ...
    public static void convertYUV420SPToARGB8888(byte[] input, int width, int height, int[] output) {
        if (useNativeConversion) {
            try {
                TFImageUtils.convertYUV420SPToARGB8888(input, output, width, height, false);
                return;
            } catch (UnsatisfiedLinkError e) {
                Log.w(TAG,
...
//        matrix.postScale(scaleFactor, scaleFactor);
                ...
                // BF Change: Scale by minimum factor so all of src present in dst while
                // maintaining the aspect ration. Some of the dst may not be filled.
