    ...
    override suspend fun doWork(): Result {
        ...
        if (readingsLeftToUpload > 0) {
            ...
            if (readingResult is Success) {
                ...
                // TODO: https://csil-git1.cs.surrey.sfu.ca/415-cradle/cradle-platform/-/blob/master/server/api/resources/sync.py#L97-103
                //  The only reason why a reading might still not be uploaded but the response from
                //  server is still successful is that the server will skip readings that are for
                ...
                //      ...
                //  This needs to be resolved somehow, although we should note that this usually
                //  never happens, because it requires the Android client to somehow upload a
                //  reading for a patient that is not on the server; this is not really possible,
                //  because patients on Android are synced first before reading sync. Also, any
                //  patients that are sent through SMS are still treated as unuploaded in case the
                //  SMS did not actually reach the server.
            }
        }
